...
#define ES_SYSTEM_VERSION_GREATER_THAN_OR_EQUAL_TO(v)  ([[[UIDevice currentDevice] systemVersion] compare:v options:NSNumericSearch] != NSOrderedAscending)
...
#define ESOpenUDIDLog(fmt, ...)
//#define ESOpenUDIDLog(fmt, ...) NSLog((@"%s [Line %d] " fmt), __PRETTY_FUNCTION__, __LINE__, ##__VA_ARGS__);
//#define ESOpenUDIDLog(fmt, ...) NSLog((@"[Line %d] " fmt), __LINE__, ##__VA_ARGS__);

static NSString * kESOpenUDIDSessionCache = nil;
static NSString * const kESOpenUDIDDescription = @"OpenUDID_with_iOS6_Support";
static NSString * const kESOpenUDIDKey = @"OpenUDID";
static NSString * const kESOpenUDIDSlotKey = @"OpenUDID_slot";
static NSString * const kESOpenUDIDAppUIDKey = @"OpenUDID_appUID";
static NSString * const kESOpenUDIDTSKey = @"OpenUDID_createdTS";
static NSString * const kESOpenUDIDOOTSKey = @"OpenUDID_optOutTS";
static NSString * const kESOpenUDIDDomain = @"org.OpenUDID";
static NSString * const kESOpenUDIDSlotPBPrefix = @"org.OpenUDID.slot.";
static int const kESOpenUDIDRedundancySlots = 100;
...
+ (void) _setDict:(id)dict forPasteboard:(id)pboard {
#if TARGET_OS_IPHONE || TARGET_IPHONE_SIMULATOR
        [pboard setData:[NSKeyedArchiver archivedDataWithRootObject:dict] forPasteboardType:kESOpenUDIDDomain];
#else
        [pboard setData:[NSKeyedArchiver archivedDataWithRootObject:dict] forType:kESOpenUDIDDomain];
#endif
}
...
+ (NSMutableDictionary*) _getDictFromPasteboard:(id)pboard {
#if TARGET_OS_IPHONE || TARGET_IPHONE_SIMULATOR
        id item = [pboard dataForPasteboardType:kESOpenUDIDDomain];
#else
        id item = [pboard dataForType:kESOpenUDIDDomain];
#endif
        if (item) {
                @try{
                        item = [NSKeyedUnarchiver unarchiveObjectWithData:item];
                } @catch(NSException* e) {
                        ESOpenUDIDLog(@"Unable to unarchive item %@ on pasteboard!", [pboard name]);
                        item = nil;
                }
        }
...
+ (NSString*) valueWithError:(NSError **)error {
        ...
        if (kESOpenUDIDSessionCache!=nil) {
                if (error!=nil)
                        *error = [NSError errorWithDomain:kESOpenUDIDDomain
                                                     code:kESOpenUDIDErrorNone
                                                 userInfo:[NSDictionary dictionaryWithObjectsAndKeys:@"OpenUDID in cache from first call",@"description", nil]];
                return kESOpenUDIDSessionCache;
        }
        ...
        //
        NSString * appUID = [defaults objectForKey:kESOpenUDIDAppUIDKey];
        if(appUID == nil)
        {
        ...
        //
        id localDict = [defaults objectForKey:kESOpenUDIDKey];
        if ([localDict isKindOfClass:[NSDictionary class]]) {
                ...
                openUDID = [localDict objectForKey:kESOpenUDIDKey];
                myRedundancySlotPBid = [localDict objectForKey:kESOpenUDIDSlotKey];
                optedOutDate = [localDict objectForKey:kESOpenUDIDOOTSKey];
                optedOut = optedOutDate!=nil;
                ESOpenUDIDLog(@"localDict = %@",localDict);
        }
        ...
        NSMutableDictionary* frequencyDict = [NSMutableDictionary dictionaryWithCapacity:kESOpenUDIDRedundancySlots];
        for (int n=0; n<kESOpenUDIDRedundancySlots; n++) {
                NSString* slotPBid = [NSString stringWithFormat:@"%@%d",kESOpenUDIDSlotPBPrefix,n];
#if TARGET_OS_IPHONE || TARGET_IPHONE_SIMULATOR
                UIPasteboard* slotPB = [UIPasteboard pasteboardWithName:slotPBid create:NO];
#else
                NSPasteboard* slotPB = [NSPasteboard pasteboardWithName:slotPBid];
#endif
                ESOpenUDIDLog(@"SlotPB name = %@",slotPBid);
                if (slotPB==nil) {
                        ...
                        NSString* oudid = [dict objectForKey:kESOpenUDIDKey];
                        ESOpenUDIDLog(@"SlotPB dict = %@",dict);
                        if (oudid==nil) {
                        ...
                        NSString* gid = [dict objectForKey:kESOpenUDIDAppUIDKey];
                        if (gid!=nil && [gid isEqualToString:appUID]) {
                                ...
                                if (optedOut) {
                                        optedOutDate = [dict objectForKey:kESOpenUDIDOOTSKey];
                                        optedOut = optedOutDate!=nil;
                                }
                        }
                }
        ...
        if (openUDID==nil) {
                ...
                if (localDict==nil) {
                        ...
                        [localDict setObject:openUDID forKey:kESOpenUDIDKey];
                        [localDict setObject:appUID forKey:kESOpenUDIDAppUIDKey];
                        [localDict setObject:[NSDate date] forKey:kESOpenUDIDTSKey];
                        if (optedOut) [localDict setObject:optedOutDate forKey:kESOpenUDIDTSKey];
        ...
        //
        ESOpenUDIDLog(@"Available Slot %@ Existing Slot %@",availableSlotPBid,myRedundancySlotPBid);
        if (availableSlotPBid!=nil && (myRedundancySlotPBid==nil || [availableSlotPBid isEqualToString:myRedundancySlotPBid])) {
                ...
                //
                if (localDict) {
                        [localDict setObject:availableSlotPBid forKey:kESOpenUDIDSlotKey];
                        saveLocalDictToDefaults = YES;
                }
        ...
        if (localDict && saveLocalDictToDefaults)
                [defaults setObject:localDict forKey:kESOpenUDIDKey];
        ...
        //
        if (optedOut) {
                if (error!=nil) *error = [NSError errorWithDomain:kESOpenUDIDDomain
                                                             code:kESOpenUDIDErrorOptedOut
                ...
                kESOpenUDIDSessionCache = [NSString stringWithFormat:@"%040x",0];
                return kESOpenUDIDSessionCache;
        }
        ...
        if (error!=nil) {
                if (isCompromised)
                        *error = [NSError errorWithDomain:kESOpenUDIDDomain
                                                     code:kESOpenUDIDErrorCompromised
        ...
        kESOpenUDIDSessionCache = openUDID;
        return kESOpenUDIDSessionCache;
}
...
+ (void) setOptOut:(BOOL)optOutValue {
        ...
        id dict = [defaults objectForKey:kESOpenUDIDKey];
        ...
        if (optOutValue)
                [dict setObject:[NSDate date] forKey:kESOpenUDIDOOTSKey];
        else
                [dict removeObjectForKey:kESOpenUDIDOOTSKey];
        
        // store the dictionary locally
        [defaults setObject:dict forKey:kESOpenUDIDKey];
        
        ESOpenUDIDLog(@"Local dict after opt-out = %@",dict);
        
        // reset memory cache 
        kESOpenUDIDSessionCache = nil;
